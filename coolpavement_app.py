...
for location in set(locations.values()):
    temp_dfs = [data_store[k]['Temperature'] for k, v in locations.items() if v == location and k in data_store]
    temp_c_dfs = [data_store[k]['Temperature_c'] for k, v in locations.items() if v == location and k in data_store]
    if temp_dfs:
