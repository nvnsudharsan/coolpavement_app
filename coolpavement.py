...
# Add sunrise and sunset times to the plots with white for day and grey for night
...
for i, date in enumerate(date_list):
    ...
    # White for daytime
    fig2.add_vrect(x0=sunrise_time, x1=sunset_time, fillcolor="#FFFFFF", opacity=0.3, layer="below", line_width=0)
    
    # Grey for nighttime before sunrise
    if i == 0:
        fig2.add_vrect(x0=daily_profile.index[0], x1=sunrise_time, fillcolor="#808080", opacity=0.3, layer="below", line_width=0)
    
    # Grey for nighttime after sunset
    if i != len(date_list) - 1:
        next_sunrise_time, _ = get_sun_rise_set_time(date_list[i+1])
        fig2.add_vrect(x0=sunset_time, x1=next_sunrise_time, fillcolor="#808080", opacity=0.3, layer="below", line_width=0)
    else:
        fig2.add_vrect(x0=sunset_time, x1=daily_profile.index[-1], fillcolor="#808080", opacity=0.3, layer="below", line_width=0)
